...
export interface SupplierProps extends SupplierFields {
  id: string;
  tenantId: string;
  companyId: string;
  code: string;
...
/** Company-scoped master data (MASTER_SPEC Fase 2). Kept separate from Customer — see the schema.prisma docstring on `Customer` for why. */
export class Supplier {
  ...
  static create(props: SupplierProps): Supplier {
    ...
    return new Supplier({ ...props, code, name });
  }
  ...
  update(name: string, fields: SupplierFields): void {
  ...
  toProps(): Readonly<SupplierProps> {
    return { ...this.props };
  }
