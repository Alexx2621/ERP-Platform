...
export class ListInventoryMovementsQueryDto {
  ...
  @ApiPropertyOptional({ enum: ["TRANSFER", "RESERVATION", "MANUAL"] })
  @IsOptional()
  @IsIn(["TRANSFER", "RESERVATION", "MANUAL"])
...
export class InventoryMovementResponseDto {
  ...
  @ApiProperty({
    enum: ["RECEIPT", "ISSUE", "ADJUSTMENT", "TRANSFER_OUT", "TRANSFER_IN", "TRANSFER_CANCELLED", "RESERVATION", "RELEASE"],
  })
  type!: string;
  @ApiProperty({ example: "-5.0000", description: "Signed — the exact delta this row applied." }) quantity!: string;
  @ApiProperty({ type: String, nullable: true }) reason!: string | null;
  @ApiProperty({ type: String, nullable: true, enum: ["TRANSFER", "RESERVATION", "MANUAL"] })
