...
import { ProductionOrderNotDraftError, ProductionOrderNotFoundError } from "../errors";
...
@Injectable()
export class ConfirmProductionOrderUseCase {
  constructor(@Inject(PRODUCTION_ORDER_REPOSITORY) private readonly productionOrders: ProductionOrderRepository) {}

  async execute(input: ConfirmProductionOrderInput): Promise<ProductionOrder> {
